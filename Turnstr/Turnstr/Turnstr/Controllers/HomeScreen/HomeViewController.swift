...
class HomeViewController: ParentViewController {
        ...
        objDataS.getUserProfileData { (received) in
            if received == true {
                DispatchQueue.main.async {
                    ...
                    let attStr = strBio.attributtedString(appendString: "\(str2)\(self.objSing.strUserWebsite)", color1: .black, color2: kBlueColor, font1: UIFont.systemFont(ofSize: 14.0, weight: UIFont.Weight.regular), font2: UIFont.systemFont(ofSize: 12.0, weight: UIFont.Weight.regular), lineSpacing: 1, alignment: .center)
                    ...
                    if let strBgImage = self.objSing.bgImage, let url = strBgImage.url {
                        self.bgImage.sd_setImage(with: url)
                    }
    ...
    func FollowerCounts() {
        ...
        postTitle1.addAttribute(NSAttributedString.Key.font, value: UIFont.systemFont(ofSize: 10), range: NSMakeRange(0, postTitle1.length))
        postTitle1.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor.white, range: NSMakeRange(0, postTitle1.length))
        
        let style1 = NSMutableParagraphStyle()
        style1.alignment = .right
        postTitle1.addAttribute(NSAttributedString.Key.paragraphStyle, value: style1, range: NSMakeRange(0, postTitle1.length))
        ...
        postDetail.addAttribute(NSAttributedString.Key.paragraphStyle, value: style1, range: NSMakeRange(0, postDetail.length))
    ...
    @objc func MyStoryClicked(sender: UIButton) {
        ///Open Camera
