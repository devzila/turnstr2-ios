...
//        return
        ...
        if let accessToken = AccessToken.current {
            kAppDelegate.loadingIndicationCreationMSG(msg: "Login")
            self.APIRequest(sType: kAPIFacebookLogin, data: ["user_id": accessToken.userID,
                                                             "access_token": accessToken.tokenString])
            return
        }
        loginManager.logIn(permissions: [Permission.publicProfile, .userFriends, .email], viewController: self) { (loginResult) in

            print("LoginResult---\(loginResult)")
            if let currentUser = AccessToken.current {
                kAppDelegate.loadingIndicationCreationMSG(msg: "Login")
                self.APIRequest(sType: kAPIFacebookLogin, data: ["user_id": currentUser.userID,
                                                                 "access_token": currentUser.tokenString])
            }
        }
    }
