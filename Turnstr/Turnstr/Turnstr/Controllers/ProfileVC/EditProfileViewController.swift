...
class EditProfileViewController: ParentViewController, UITableViewDelegate, UITableViewDataSource, EditProfileHeaderDelegate {
    ...
    var btnChangeBackgroundImage = UIButton(type: .custom)
    ...
    func SetFieldsSetting() -> Void {
        ...
        let attributedString = NSAttributedString(string: "Change Background Image",
                                                  attributes: [NSAttributedString.Key.font: font,
                                                               NSAttributedString.Key.underlineColor: kOrangeColor,
                                                               NSAttributedString.Key.underlineStyle: 1,
                                                               NSAttributedString.Key.foregroundColor: kOrangeColor])
        btnChangeBackgroundImage.setAttributedTitle(attributedString, for: .normal)
        btnChangeBackgroundImage.contentHorizontalAlignment = .left
        btnChangeBackgroundImage.addTarget(self, action: #selector(changeBackgroundImage), for: .touchUpInside)
    ...
    @objc private func changeBackgroundImage() {
        CameraImage.shared.captureImage(from: self, captureOptions: [.camera, .photoLibrary], allowEditting: false, fileTypes: [.image]) { (image, _) in
            if let img = image {
                self.uploadCoverPic(uploadedImage: img)
            }
        }
    }
    //MARK:- Table View
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        if indexPath.section == 0 {
            ...
            case 3:
                cell?.imgPic?.setImage(url: objSing.bgImage.url)
                btnChangeBackgroundImage.frame = frame
                cell.uvContent.addSubview(btnChangeBackgroundImage)
            case 4:
    ...
    @objc func DoneClicked(sender: UIButton) -> Void {
    ...
    @objc func ChangeProfilePhoto(sender: UIButton) -> Void {
    ...
    private func uploadCoverPic(uploadedImage: UIImage) {
        ...
        DispatchQueue.global().async {
            let arrResponse = self.objDataS.uploadFilesToServer(dictAction: ["action": kAPIUploadBackgroundPic],
                                                                arrImages: [uploadedImage])
            if (arrResponse.count) > 0 {
                DispatchQueue.main.async {
