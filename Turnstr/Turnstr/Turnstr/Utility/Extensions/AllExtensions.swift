...
extension UIImageView {
    ...
    func setImage(url: URL?, placeholder: UIImage = #imageLiteral(resourceName: "thumb")) {
        if let url = url {
            sd_setImage(with: url, placeholderImage: placeholder, options: .highPriority)
        }
        else {
            image = placeholder
        }
...
extension UIImage {
    ...
    func setMode(MODE: UIImage.RenderingMode) -> UIImage {
...
extension String {
    ...
    func attributtedString(appendString: String, color1: UIColor, color2: UIColor, font1: UIFont, font2: UIFont, lineSpacing: CGFloat = 0, alignment: NSTextAlignment = .left) -> NSAttributedString {
        ...
        //Font
        attString.addAttribute(NSAttributedString.Key.font, value: font1, range: NSMakeRange(0, (self.count)))
        attString.addAttribute(NSAttributedString.Key.font, value:font2, range: NSMakeRange(fullString.count-strSubTitle.count, strSubTitle.count))
        
        //Color
        attString.addAttribute(NSAttributedString.Key.foregroundColor, value: color1, range: NSMakeRange(0, attString.length))
        attString.addAttribute(NSAttributedString.Key.foregroundColor, value: color2, range: NSMakeRange(attString.length - strSubTitle.count, strSubTitle.count))
