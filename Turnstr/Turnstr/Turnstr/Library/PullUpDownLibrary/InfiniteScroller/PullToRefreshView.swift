...
open class PullToRefreshView: UIView {
   ...
   private var kvoContext = "PullToRefreshKVOContext"
    ...
    var state: PullToRefreshState = PullToRefreshState.pulling {
        didSet {
            ...
            case .pulling: //starting point
                if self.hasMoreData { arrowRotationBack()   } else {
    ...
    init(options: PullToRefreshOption, frame: CGRect, refreshCompletion: RefreshCompletion?, down: Bool=true) {
    ...
    open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        setAlphaForArrow(offsetY: offsetY)
        if offsetY <= 0 {
            if !self.pull {
                return
            }
        ...
        pushUp(upHeight: upHeight, scrollView: scrollView)

    }
    func setAlphaForArrow(offsetY: CGFloat) {
        // Alpha set
    ...
    func pushUp(upHeight: CGFloat, scrollView: UIScrollView) {
        if upHeight > 0 {
            // pulling or refreshing
            if self.pull {
                return
