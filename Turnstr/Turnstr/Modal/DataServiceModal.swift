...
class DataServiceModal: NSObject {
    ...
    func uploadFilesToServer(dictAction:NSDictionary, arrImages: Array<UIImage>) -> Dictionary<String,AnyObject>  {
        ...
        else if action == kAPIUploadBackgroundPic {
            strPostUrl = kAPIUploadBackgroundPic
            dictRequest = dictAction as! [String : String]
        }
                ...
                if let statusCode = dictResponse["statusCode"] as? Int, statusCode == 200 {
                    ...
                    if let dictComments = dictResponse["data"]?["data"] as? [String: AnyObject] {
                        if let objUser = dictComments["user"] as? [String: Any] {
                            ...
                            if let bgImage = objUser["background_image_url"] as? String {
                                self.objSingl?.bgImage = bgImage
                            }
                        }
