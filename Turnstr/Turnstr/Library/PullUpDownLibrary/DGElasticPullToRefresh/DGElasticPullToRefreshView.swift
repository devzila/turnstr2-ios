...
// MARK: Constant
private  let minOffsetToPull: CGFloat = 95.0
private  let loadingContentInset: CGFloat = 50.0
private  let loadingViewSize: CGFloat = 30.0
private  let waveMaxHeight: CGFloat = 70.0

// MARK:  Observer

private let contentOffset = "contentOffset"
private let contentInset = "contentInset"
private let frameOfpullToRefresh = "frame"
private let panGestureRecognizerState = "panGestureRecognizer.state"
...
open class DGElasticPullToRefreshView: UIView {
    ...
    fileprivate var stateVal: DGElasticPullToRefreshState = .stopped
    ...
    var observing: Bool = false {
        didSet {
            ...
            if observing {
                scrollView.dg_addObserver(self, forKeyPath: contentOffset)
                scrollView.dg_addObserver(self, forKeyPath: contentInset)
                scrollView.dg_addObserver(self, forKeyPath: frameOfpullToRefresh)
                scrollView.dg_addObserver(self, forKeyPath: panGestureRecognizerState)
            } else {
                scrollView.dg_removeObserver(self, forKeyPath: contentOffset)
                scrollView.dg_removeObserver(self, forKeyPath: contentInset)
                scrollView.dg_removeObserver(self, forKeyPath: frameOfpullToRefresh)
                scrollView.dg_removeObserver(self, forKeyPath: panGestureRecognizerState)
            }
        }
    }
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        } else if keyPath == frameOfpullToRefresh {
            layoutSubviews()
        } else if keyPath == panGestureRecognizerState {
    ...
    fileprivate func currentWaveHeight() -> CGFloat {
        return min(bounds.height / 3.0 * 1.6, waveMaxHeight)
    }
    ...
    fileprivate func scrollViewDidChangeContentOffset(dragging: Bool) {
        ...
        if state == .stopped && dragging {  state = .dragging } else if state == .dragging && dragging == false {
            if offsetY >= minOffsetToPull {
    ...
    fileprivate func resetScrollViewContentInset(shouldAddObserverWhenFinished: Bool, animated: Bool, completion: (() -> ())?) {
        ...
        var contentInsetValue = scrollView.contentInset
        contentInsetValue.top = originalContentInsetTop

        if state == .animatingBounce {
            contentInsetValue.top += currentHeight()
        } else if state == .loading {
            contentInsetValue.top += loadingContentInset
        }

        scrollView.dg_removeObserver(self, forKeyPath: contentInset)

        let animationBlock = { scrollView.contentInset = contentInsetValue }
    ...
    fileprivate func animateBounce() {
       ...
       resetScrollViewContentInset(shouldAddObserverWhenFinished: false, animated: false, completion: nil)
        ...
        let centerY = loadingContentInset
        let duration = 0.9
        ...
        UIView.animate(withDuration: duration, delay: 0.0, usingSpringWithDamping: 0.43, initialSpringVelocity: 0.0, options: [], animations: { [weak self] in
            ...
            self?.r3ControlPointView.center.y = centerY
                ...
                if let strongSelf = self, let scrollView = strongSelf.scrollView() {
                    scrollView.dg_addObserver(strongSelf, forKeyPath: contentOffset)
        ...
        UIView.animate(withDuration: duration * 0.4, animations: { [weak self] in
            if let contentInsetTop = self?.originalContentInsetTop {
                self?.bounceAnimationHelperView.center = CGPoint(x: 0.0, y: contentInsetTop + loadingContentInset)
            }
            }, completion: nil)
    }
    ...
    fileprivate func layoutLoadingView() {
        ...
        let loadingViewSizeValue: CGFloat = loadingViewSize
        let minOriginY = (loadingContentInset - loadingViewSizeValue) / 2.0
        let originY: CGFloat = max(min((height - loadingViewSizeValue) / 2.0, minOriginY), 0.0)

        loadingView?.frame = CGRect(x: (width - loadingViewSizeValue) / 2.0, y: originY, width: loadingViewSizeValue, height: loadingViewSizeValue)
