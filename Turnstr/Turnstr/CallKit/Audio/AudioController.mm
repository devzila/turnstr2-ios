...
- (void)handleRouteChange:(NSNotification *)notification
{
    ...
    NSLog(@"Reason value:  %hhu" , reasonValue);
    NSLog(@"Route change:");
    switch (reasonValue) {
...
{
    NSLog(@"Media server has reset %@", notification);
...
- (void)setupAudioSession
{
    try {
       ...
       // [sessionInstance setMode:AVAudioSessionModeVideoChat error:&error];
      ...
      //  AVF_EXPORT NSString *const AVAudioSessionPortHeadsetMic   NS_AVAILABLE_IOS(6_0);
