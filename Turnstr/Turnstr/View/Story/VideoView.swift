...
class VideoView: UIView, AVCaptureFileOutputRecordingDelegate, VideoPlayerDelegates {
    ...
    func cameraSetup(){
        ...
        let devices: NSArray = AVCaptureDevice.devices(for: AVMediaType.audio) as NSArray
    ...
    func initSessionWithCamera(cameraType: Camera){
        ...
        let device = self.deviceWithMediaTypeWithPosition(mediaType: AVMediaType.video.rawValue, cameraType: cameraType)
        do {
    ...
    func deviceWithMediaTypeWithPosition(mediaType: String, cameraType: Camera) -> AVCaptureDevice {
        let devices: NSArray = AVCaptureDevice.devices(for: AVMediaType(rawValue: mediaType)) as NSArray
...
//        exportVideo(outputUrl: outputFileURL as NSURL, ofDuration: captureAudio.re)
        self.exportVideo(outputUrl: outputFileURL as NSURL, ofDuration: output.recordedDuration)
    ...
    func exportVideo(outputUrl: NSURL, ofDuration duration: CMTime){
        ...
        if asset.tracks(withMediaType: AVMediaType.video).isEmpty{
